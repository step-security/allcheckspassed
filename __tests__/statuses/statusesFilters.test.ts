...
describe("statusesFilters", () => {
  ...
  describe("getMostRecentStatusPerContextAndCreator", () => {
    it("should return the status with highest ID for each context/creator combination", () => {
      ...
      // Should include the highest ID for step-security
      const stepsecurityStatus = result.find((s) => s.creator.login === "step-security");
      expect(stepsecurityStatus).toBeDefined();
      expect(stepsecurityStatus?.id).toBe(39672993325);
      expect(stepsecurityStatus?.state).toBe("failure");

      // Should include the highest ID for step-security-app[bot]
      const botStatus = result.find(
        (s) => s.creator.login === "step-security-app[bot]"
      );
